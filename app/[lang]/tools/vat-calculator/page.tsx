...
const seoContent: Record<Locale, { title: string; paragraphs: string[]; faq: { q: string; a: string }[] }> = {
  en: {
    title: 'Free VAT Calculator — How to Calculate Value Added Tax Online',
    paragraphs: [
      'Value Added Tax (VAT) is a consumption tax applied at each stage of the supply chain where value is added to a product or service. Unlike a simple sales tax, which is only collected at the final point of sale, VAT is levied incrementally throughout the production and distribution process. For businesses, consumers, and freelancers, understanding the exact VAT amount on every transaction is essential for accurate bookkeeping, pricing strategies, and tax compliance. Whether you need to add VAT to a net price before issuing a quote or remove VAT from a gross receipt to find the original cost, a reliable VAT calculator saves you from manual arithmetic and potential errors. Governments worldwide rely on VAT because it generates steady revenue and is harder to evade than single-stage taxes.',
      'Our free online VAT calculator lets you instantly calculate VAT on any amount. Simply type in your figure, select the applicable VAT rate for your country, and choose whether you want to add VAT or remove VAT. The tool displays the net amount, the VAT portion, and the gross total in a clear visual breakdown, so you can verify your numbers at a glance. It supports the standard and reduced rates used across the UK (20%, 5%), Italy (22%, 10%, 5%, 4%), Spain (21%, 10%, 4%), France (20%, 10%, 5.5%, 2.1%), Germany (19%, 7%), and Portugal (23%, 13%, 6%), making it invaluable for cross-border trade within Europe.',
      '<strong>How to calculate VAT — step-by-step formulas.</strong> To add VAT to a net price, multiply the net amount by the VAT rate and add the result: Gross = Net x (1 + VAT rate). For example, if your net price is 500 and the VAT rate is 20%, the VAT amount is 500 x 0.20 = 100, and the gross total is 600. To remove VAT from a gross price (reverse VAT calculation), divide the gross amount by (1 + VAT rate): Net = Gross / (1 + VAT rate). So a gross price of 600 at 20% gives a net price of 600 / 1.20 = 500. To find just the VAT portion from a VAT-inclusive price, use: VAT = Gross - (Gross / (1 + VAT rate)), or equivalently VAT = Gross x (VAT rate / (1 + VAT rate)). These formulas work for any rate — just substitute 20% with your country\'s applicable rate.',
      'VAT rates vary significantly from country to country, and knowing which rate applies is critical for correct invoicing. In the United Kingdom the standard VAT rate is 20%, with a reduced rate of 5% for domestic fuel and certain child products, while some goods such as most food items and children\'s clothing are zero-rated. Germany applies a 19% standard rate and a 7% reduced rate for groceries, books, and public transport. France has four tiers: 20% standard, 10% intermediate, 5.5% reduced, and 2.1% super-reduced for newspapers and pharmaceuticals. The Netherlands charges 21% standard and 9% reduced, Ireland uses 23% and 13.5%, and Nordic countries like Sweden and Denmark set their standard rates at 25%. Outside Europe, countries such as Australia (10% GST), Canada (5% federal GST plus provincial rates), India (5%-28% GST slabs), and South Africa (15%) also operate value-added tax systems under different names. Always confirm the current rate with your national tax authority before filing returns.',
      'Accurate VAT calculations help businesses avoid underpaying or overpaying tax, which can result in penalties from tax authorities. If you are preparing invoices, our <a href="/en/tools/invoice-generator">invoice generator</a> pairs perfectly with this tool: compute the VAT here, then build your complete invoice in seconds. For more general percentage work, try our <a href="/en/tools/percentage-calculator">percentage calculator</a>, and if you deal with US-style taxes, our <a href="/en/tools/sales-tax-calculator">sales tax calculator</a> covers that side of things. Combining these tools gives you a complete financial arithmetic toolkit without the need for spreadsheets or paid accounting software.',
      'A before-VAT calculator is particularly useful for freelancers and sole traders who receive gross payments and need to determine how much of that payment is tax. By using the "Remove VAT" mode, you can quickly find the net income and the VAT you owe to the tax office. This is also helpful when comparing prices across countries with different VAT rates, planning budgets for imported goods, or verifying supplier invoices. E-commerce sellers who ship across borders rely on reverse VAT calculations to set competitive prices while remaining compliant with destination-country tax rules. The calculation history feature lets you review your last five conversions without re-entering data, and the copy button makes it easy to paste results into spreadsheets, emails, or accounting software.',
      'Whether you are a small business owner calculating value added tax on your products, an accountant verifying client invoices, or an individual trying to understand the tax breakdown on a purchase, this VAT calculator provides the accuracy and speed you need. It handles both VAT-exclusive and VAT-inclusive calculations, supports multiple country rates, and requires no registration or downloads. Bookmark this page and use it whenever you need to calculate VAT quickly and correctly.',
    ],
    faq: [
      { q: 'How do I calculate VAT from a gross amount?', a: 'To extract VAT from a gross (tax-inclusive) price, divide the gross amount by (1 + VAT rate). For example, if the gross price is 120 and VAT is 20%, the net amount is 120 / 1.20 = 100, and the VAT portion is 20. This is sometimes called a "reverse VAT calculation" or "before VAT calculator" because you are working backwards from the final price to find the original amount before tax was applied. The formula works for any VAT rate: simply replace 20% with your applicable rate.' },
      { q: 'What is the difference between adding and removing VAT?', a: 'Adding VAT means computing the tax on top of a net (tax-exclusive) price to get the gross total. For instance, a net price of 100 with 20% VAT becomes 120. Removing VAT means extracting the tax portion from a gross price to find the original net amount. Both operations use the same VAT rate but apply the formula differently. Use "Add VAT" when pricing products and issuing quotes, and "Remove VAT" when analysing receipts, verifying costs, or determining your net income from a gross payment.' },
      { q: 'Which VAT rates apply in European countries?', a: 'Each EU and EEA country sets its own standard and reduced VAT rates. The UK uses 20% standard and 5% reduced, Italy has 22% standard with reduced rates of 10%, 5%, and 4%, Germany applies 19% and 7%, France has 20%, 10%, 5.5%, and 2.1%, Spain uses 21%, 10%, and 4%, Portugal applies 23%, 13%, and 6%, the Netherlands charges 21% and 9%, Ireland uses 23% and 13.5%, and Nordic countries such as Sweden and Denmark set their standard rate at 25%. Always verify the latest rates with your national tax authority, as governments may adjust them.' },
      { q: 'Can I use this VAT calculator for business invoices?', a: 'Absolutely. Enter the net amount of your product or service, select the appropriate VAT rate, and the calculator will show you the exact VAT to charge and the gross total to include on your invoice. You can copy the result directly and use it alongside our <a href="/en/tools/invoice-generator">invoice generator</a> to create professional invoices, or pair it with our <a href="/en/tools/percentage-calculator">percentage calculator</a> for discount scenarios. This workflow ensures every invoice you send has the correct tax breakdown, helping you stay compliant and professional.' },
      { q: 'Is VAT the same as sales tax?', a: 'No, they differ in structure. VAT is collected at every stage of production and distribution, with each business in the chain remitting tax only on the value it adds. Sales tax is collected only once at the final point of sale to the consumer. VAT is the standard model in over 160 countries including the entire EU, the UK, Canada (GST), and Australia (GST), while sales tax is primarily used in the United States. If you need to calculate US-style sales tax, try our dedicated <a href="/en/tools/sales-tax-calculator">sales tax calculator</a> tool.' },
      { q: 'How do I calculate the VAT percentage between two amounts?', a: 'If you know the net (before tax) and gross (after tax) amounts and want to find the VAT rate that was applied, use this formula: VAT rate = ((Gross - Net) / Net) x 100. For example, if the net price is 200 and the gross price is 240, the VAT amount is 40, and the rate is (40 / 200) x 100 = 20%. This is useful when you receive an invoice that shows totals but does not explicitly state the VAT percentage, or when you need to verify that the correct rate was charged.' },
    ],
  },
  it: {
